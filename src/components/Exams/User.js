  ...
  return (
    <div className="profile-container">
        ...
        {candidates.length > 0 ? (
          candidates.map((candidate) => (
            <div key={candidate.email} className="candidate-card">
              <p className="candidateemail"><strong> <Mail /> </strong> {candidate.email}</p>
              <p><strong>Score:</strong> {candidate.score ?? "Pending"}</p>
            </div>
          ))
        ) : (
      ...
      <div className="pending-list">
        {candidates.filter(c => c.score === null || c.score === undefined).length > 0 ? (
          candidates
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <div className="modal-actions">
              ...
              <button className="confirm-btn" onClick={handleAddCandidate}>Add</button>
            </div>
          </div>
        </div>
      )}
